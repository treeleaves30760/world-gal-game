...
portrait_backend / ambient_backend。第三方主題（恐怖、戀愛、養成…）、自訂遊戲機制、
AI 工具產生的新功能 — 都應該走插件。

十種擴充點（Phase 1 開了 4 個，Phase 2 補齊 4 個，Phase 5A 加上 portrait backend，
Phase 6 表演層加上 ambient backend）：
...
| `@ambient_backend(name)` | 環境 / 天氣 overlay 後端（雨雪花瓣…） | 6 |
...
### `@ambient_backend(name, *, description="", plugin_id=None)`  *(Phase 6)*

Class decorator：註冊一個「環境 / 天氣 overlay 後端」—— 雨、雪、花瓣、光點、螢火等
**全螢幕氛圍層**，畫在世界層（背景 + CG + 立繪）之上、文字框之下。由 `set_weather`
effect 開啟、`clear_weather` 關閉（兩者都走和 camera/screen FX 同一條 directive
佇列）。未註冊的後端名會優雅退回「無 overlay」，所以缺插件不會弄壞畫面。

後端 class 由對話場景實例化為 `cls(params, screen_size)`，需實作兩個方法（見
`world_gal_game/ui/ambient_backend.py`）。**務必確定性**（用內建 `Lcg` 種子化，不要
用全域 `random`），這樣存檔重播 / 截圖才可重現同一幀：

```python
from world_gal_game.plugins import ambient_backend
from world_gal_game.ui.ambient_backend import ParticleBackend, Lcg, fnum

@ambient_backend("rain", description="斜向落下的雨絲。")
class RainBackend(ParticleBackend):     # ParticleBackend 處理 spawn/積分/環繞
    default_count = 220
    def __init__(self, params=None, screen_size=(1280, 720)):
        params = dict(params or {}); params.setdefault("alpha", 160)
        self._speed = fnum(params, "speed", 900.0)   # 在 super() 之前解析,
        super().__init__(params, screen_size)         # 因為 super 會呼叫 _spawn
    def _spawn(self, rng: Lcg, w, h):     # 一顆粒子的初始狀態
        return {"x": rng.uniform(-w*0.3, w), "y": rng.uniform(-h, h),
                "vx": 0.0, "vy": self._speed}
    def _draw(self, surface, p):          # 畫一顆粒子
        ...
```

`ParticleBackend` 讀的共通 `params`：`count`（粒子數）、`seed`（RNG 種子）、
`alpha`（0-255 整體不透明度）;其餘鍵由各後端自取（`WeatherValue` 不認得的鍵在 lint
時被忽略,但執行期仍原樣傳入）。

```yaml
# 場景 YAML — 對白 line 的 effects
- text: "（窗外下起了雨。）"
  effects:
    - kind: set_weather
      target: rain                 # 指名已註冊的 @ambient_backend
      value: {count: 200, wind: -240, fade: 1.5}   # fade = 淡入秒數
- text: "（雨停了。）"
  effects:
    - kind: clear_weather
      value: {fade: 1.0}           # 淡出秒數
...
引擎內建一個 **`ambient_weather`** 插件（`world_gal_game/plugins_user/`），提供五個
web-safe（純 pygame、確定性、桌面/web 一致）後端：`rain`（雨絲）、`snow`（飄雪 +
橫向擺動）、`petals`（櫻花瓣翻轉）、`sparkles`（原地閃爍光點）、`fireflies`（漫遊
脈動的螢火）。權威清單見 `wgg capabilities`（`markup.ambient_backends`）。

### 影片播放器（movie player）登錄表  *(Phase 6 / 5B)*

影片播放不走 decorator 類別,而是一個輕量函式登錄表（`world_gal_game/ui/movie_player.py`
的 `register_movie_player(name, factory)`）。`play_movie` effect / `MoviePlayerScene`
依名字挑播放器;內建 `image_sequence`（純 pygame、web-safe 的連號影格播放器）。

桌面真 video 走插件:內建 **`desktop_video`** 插件（`world_gal_game/plugins_user/`）在
載入時 `register_movie_player("video", …)`,以 pyvidplayer2 播 `.mp4`/`.webm`
（lazy import,缺依賴/缺 ffmpeg 時優雅降級;`pip install "world-gal-game[video]"`）。
任何插件可照同樣方式登錄自己的播放器後端。播放器須實作 `update(dt)` /
`draw(surface)` / `done` / `skip()`。已註冊清單見 `wgg capabilities`
（`markup.movie_players`）。
