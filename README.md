...
showcase 遊戲 [清華異聞錄](../Tsinghua-Strange-Tales/README.md) 就放在
引擎倉的兄弟資料夾，是用引擎做的一款完整作品；可以直接跑起來看引擎怎麼運作。
...
# 跑一個指令腳本，然後輸出最終狀態
uv run python main.py --headless --script scripts/qingyi_full_route.json
```
...
{
  "commands": [
    {"op": "start_scene", "scene": "prologue_arrival"},
    {"op": "next", "count": 12},
...
涵蓋好感、事件記錄、條件/效果、地圖、對話引擎、成就、物品送禮、整個 showcase pack 的兩條主線。
任何引擎改動都應該守住這兩個。
...
引擎本身 MIT 授權；showcase 遊戲 `../Tsinghua-Strange-Tales/` 的劇本與美術屬於該作品，
請見該倉的 README。
